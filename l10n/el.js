OC.L10N.register(
    "password_policy",
    {
    "Password needs to be at least %s characters long" : "Ο κωδικός πρόσβασης πρέπει να είναι τουλάχιστον%sσε μήκος χαρακτήρων",
    "Password needs to contain at least one lower and one upper case character." : "Ο κωδικός πρόσβασης πρέπει να περιέχει τουλάχιστον έναν μικρό και έναν κεφαλαίο χαρακτήρα.",
    "Password needs to contain at least one numeric character." : "Ο κωδικός πρόσβασης πρέπει να περιέχει τουλάχιστον ένα αριθμητικό χαρακτήρα.",
    "Password needs to contain at least one special character." : "Το συνθηματικό πρέπει να περιέχει τουλάχιστον ένα ειδικό χαρακτήρα",
    "Password is among the 1,000,000 most common ones. Please make it unique." : "Ο κωδικός πρόσβασης είναι μεταξύ των 1.000.000 συνηθέστερων. Κάντε το μοναδικό.",
    "Password policy" : "Πολιτική συνθηματικού",
    "Minimal length" : "Ελάχιστο μέγεθος",
    "Forbid common passwords" : "Απαγόρευση κοινών συνθηματικών",
    "Enforce upper and lower case characters" : "Υποχρεωτική χρήση κεφαλαίων και πεζών χαρακτήρων",
    "Enforce numeric characters" : "Υποχρεωτική χρήση αριθμητικών χαρακτήρων",
    "Enforce special characters" : "Υποχρεωτική χρήση ειδικών χαρακτήρων",
    "Password need to be at least %s characters long" : "Το συνθηματικό πρέπει να έχει μέγεθος τουλάχιστον %s χαρακτήρες",
    "Password need to contain at least one lower case character and one upper case character." : "Το συνθηματικό πρέπει να περιέχει τουλάχιστον ένα πεζό και ένα κεφαλαίο χαρακτήρα.",
    "Password need to contain at least one numeric character." : "Το συνθηματικό πρέπει να περιέχει τουλάχιστον έναν αριθμητικό χαρακτήρα.",
    "Password need to contain at least one special character." : "Το συνθηματικό πρέπει να περιέχει τουλάχιστον ένα ειδικό χαρακτήρα",
    "Password is within the 1,000,000 most common passwords. Please choose another one." : "Το συνθηματικό βρίσκεται μέσα στα 1.000.000 κοινά συνθηματικά. Παρακαλώ επιλέξτε άλλο."
},
"nplurals=2; plural=(n != 1);");
